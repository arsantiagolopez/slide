...
const DesktopUsers = ({
  ...
  return (
    <Flex
      {...styles.wrapper}
      width={{ base: "100%", md: isOpen ? "20vw" : "5vw" }}
      minWidth={isOpen ? "15em" : "5em"}
      height={height}
    >
      <Flex {...styles.bar} onClick={onToggle}>
        {!isOpen ? (
        ...
        {!isOpen ? (
          ...
          <Icon as={IoChevronForwardSharp} {...styles.icon} />
        )}
      </Flex>
      ...
      <Flex {...styles.content}>
        {users?.map((user) => {
          ...
          return (
            <Flex
              key={user.id}
              paddingX={{ base: "0.5em", md: !isOpen ? "22%" : "2em" }}
